function [outputImg,atScale,atDirection,allFiltered] = FrangiFilter2D(I, options)
...
%
% outputs,
%   outputImg : The vessel enhanced image (pixel is the maximum found in all scales)
%   atScale : Matrix with the scales on which the maximum intensity 
%           of every pixel is found
%   atDirection : Matrix with directions (angles) of pixels (from minor eigenvector)   
%
% Example,
...
defaultoptions = struct('FrangiScaleRange', [1 4], 'FrangiScaleRatio', 1, 'FrangiBetaOne', 0.5, 'FrangiBetaTwo', 15, 'verbose',true,'BlackWhite',true);
...
% output pixel value
if length(sigmas) > 1,
    [outputImg,atScale] = max(allFiltered,[],3);
    outputImg = reshape(outputImg,size(I));
    if(nargout>1)
        atScale = reshape(atScale,size(I));
    end
    if(nargout>2)
        atDirection = reshape(ALLangles((1:numel(I))'+(atScale(:)-1)*numel(I)),size(I));
    end
else
    outputImg = reshape(allFiltered,size(I));
    if(nargout>1)
            atScale = ones(size(I));
    end
    if(nargout>2)
        atDirection = reshape(ALLangles,size(I));
    end
end
